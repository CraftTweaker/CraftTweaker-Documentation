...
IntData implémente les interfaces suivantes. Cela signifie que toutes les méthodes disponibles peuvent également être utilisées dans cette classe.
...
| --------- | ------------- | -------------------------- |
| interne   | Indice        | Aucune description fournie |
...
```zenscript
8192.contains(données comme crafttweaker.api.data.IData);
8192.contains("Affichage");
```
