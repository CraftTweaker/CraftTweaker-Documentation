...
ShortData implémente les interfaces suivantes. Cela signifie que toutes les méthodes disponibles peuvent également être utilisées dans cette classe.
...
| --------- | ------------- | -------------------------- |
| interne   | court         | Aucune description fournie |
...
```zenscript
1058.contains(données comme crafttweaker.api.data.IData);
1058.contains("Affichage");
```
