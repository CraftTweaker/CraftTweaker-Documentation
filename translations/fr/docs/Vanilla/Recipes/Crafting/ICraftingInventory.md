...
`inventory.setStack(row, column, item)` sets the stack at the given position to the provided item. L'élément est une IItemStack. Use null if you want to clear the stack at that position.