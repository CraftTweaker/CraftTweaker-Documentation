...
## Объявление массивов
Объявлено с помощью `[` и `]`.

**Imortant**: вы *должны* инициализировать массивы что-то, даже если это пустой массив.
...
Если вы сейчас подумали "подождите, разве я не видел эти скобки раньше?", то "да", вы их видели. Запомнить `рецептов.add(выйти,[[],[]]);`? Здесь используются три массива, где каждый содержит по три объекта, чтобы определить рецепт крафта в верстаке.
