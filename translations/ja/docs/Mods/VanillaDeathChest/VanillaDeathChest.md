...
If any properties are not set via CraftTweaker, the values set in the global configuration are used. ゲームステージは順番に定義する必要があります。 Calling any of the VanillaDeathChest functions causes the specified stage to be defined if it has not been already.
