...
The item builder is to... build items (surprise!) <p> It allows you to set various properties that will change how the item behaves and what it can do. [mods.contenttweaker.item.ItemBuilder#withType](/mods/contenttweaker/API/item/ItemBuilder/#withtype) を使用して、より特殊なビルダーに切り替えることもできます。 <p> アイテムをインガムに表示するようCoTに伝えるには、 [mods.contentweaker.item.ItemBuilder#build(String)](/mods/contenttweaker/API/item/ItemBuilder/#build) を呼び出し、有効なリソースロケーションパスを指定する必要があります。
...
## Constructors
Creates a new ItemBuilder. これは _ではなく、_ ゲーム内に新しいブロックを作成することに注意してください。そのためには、 [mods.contenttweaker.item.ItemBuilder#build(String)](/mods/contenttweaker/API/item/ItemBuilder/#build) を呼び出す必要があります。
```zenscript
