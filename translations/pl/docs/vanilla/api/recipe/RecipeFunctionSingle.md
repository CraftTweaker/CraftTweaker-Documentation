...
```zenscript
crafttweaker.api.recipe.RecipeFunctionPojedyncze
```
...
```zenscript
myRecipeFunctionSingle.process(usualOut jako crafttweaker.api.item.IItemStack, wejścia jako crafttweaker.api.item.IItemStack);
```
