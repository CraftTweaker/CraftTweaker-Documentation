...
Il costruttore di oggetti è a... costruire oggetti (sorpresa!) <p> Ti permette di impostare varie proprietà che cambieranno come si comporta l'elemento e cosa può fare. È anche possibile utilizzare [mods.contenttweaker.item.ItemBuilder#withType](/mods/contenttweaker/API/item/ItemBuilder/#withtype) per passare a un costruttore più specializzato, se ce ne sono. <p> Per dire a CoT che vuoi che l'elemento appaia in ingame devi chiamare [mods.contenttweaker.item.ItemBuilder#build(String)](/mods/contenttweaker/API/item/ItemBuilder/#build) e specificare un percorso di localizzazione delle risorse valido.
