...
`inventory.setStack(row, column, item)` sets the stack at the given position to the provided item. Artikel ist ein IItemStack. Use null if you want to clear the stack at that position.