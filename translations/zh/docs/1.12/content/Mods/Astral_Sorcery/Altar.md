...
此方法将移除指定资源位置（resource location）的配方。  
你可以用以下方法获取配方的资源位置：在开启 F3 屏幕的情况下鼠标悬浮在 JEI 或星空日记中的物品图标上。  
如果配方不存在则输出错误信息。
```zenscript
...
这个方法将移除第一个在传入的祭坛等级下`输出`为传入的<a href="../../../Vanilla/Items/IItemStack/">物品堆（IItemStack）</a>的配方。 <br> 如果有多个配方可以合成传入的物品，你需要多次使用这个方法。
...
1.10 版本之后所有配方都需要资源位置作为第一个参数。  
与原版配方不同的是，这个参数不是可选参数，意味着如果你不提供这些参数会出现错误。 如果你想覆盖之前的配方，你可以直接添加一个与原来的资源位置相同的配方。  
你可以通过在 F3 菜单开启时将鼠标悬浮在 JEI 或星空日记的输出槽获取资源位置。
