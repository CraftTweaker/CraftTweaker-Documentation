...
关联数组（也称映射或字典），与普通的[数组](/AdvancedFunctions/Arrays_and_Loops/)类似，二者均可存储多个条目。 但与[数组](/AdvancedFunctions/Arrays_and_Loops/)的不同之处在于，其可以输入索引或键（即映射键）。
...
访问关联数组中的项，与访问普通[数组](/AdvancedFunctions/Arrays_and_Loops/)中的项，方法是同样的：  
`Array[index]`  
