...
括号处理器用于解决 `<tokens>` 内部的 [ZenTokens](/Dev_Area/ZenTokens/)。<br /> 为此，ZS 会将所有括号中的标记加入到一个列表中并将其经过所有已注册的括号处理器来寻找一个不返回 `null` 的处理器。<br />带注解的类只需要实现 [IBracketHandler](https://github.com/jaredlll08/CraftTweaker/blob/1.12/CraftTweaker2-API/src/main/java/crafttweaker/zenscript/IBracketHandler.java)。
