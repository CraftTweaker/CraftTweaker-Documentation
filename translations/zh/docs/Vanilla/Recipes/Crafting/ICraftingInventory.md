...
`inventory.setStack(row, column, item)` sets the stack at the given position to the provided item. 物品是一个 IItemStack。 Use null if you want to clear the stack at that position.