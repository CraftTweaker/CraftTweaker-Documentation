...
项目的基本生成器，也由 [mods.contenttweeper.item.basic 调用。ItemBuilderBasic#build](/mods/contenttweaker/API/item/basic/ItemBuilderBasic/#build)。 不具有任何特殊属性，它是存在的。
...
```zenscript
mods.contenttweeper.item.basic。ItemBuilderBasic
```
