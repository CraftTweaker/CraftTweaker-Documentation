...
IntArrayData implementa le seguenti interfacce. Ciò significa che ogni metodo presente nell'interfaccia può essere usato anche per questa classe.
...
| --------- | ----- | --------------------------- |
| interno   | int[] | Nessuna descrizione fornita |
...
```zenscript
[4, 128, 256, 1024].add(valore come crafttweaker.api.data.IData);
[4, 128, 256, 1024].add("oggi");
```
...
```zenscript
[4, 128, 256, 1024].contiene(dati come crafttweaker.api.data.IData);
[4, 128, 256, 1024].contiene("Display");
```
