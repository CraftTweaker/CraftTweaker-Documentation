...
| ----------- | -------------------------------------------------- | ---------------------------------------------------------------------- | -------- | ------------- |
| recipePath  | string                                             | recipePath The recipe name, without the resource location              | no       |               |
| seed        | [IIngredient](/vanilla/api/ingredient/IIngredient) | The seed that needs to be inserted in the Cloche's gui                 | no       |               |
| soil        | [IIngredient](/vanilla/api/ingredient/IIngredient) | The soil that this seeds need to grow on                               | no       |               |
| time        | int                                                | The time it takes for the crop to mature (without modifiers), in ticks | no       |               |
| outputs     | [IItemStack](/vanilla/api/item/IItemStack)[]       | The outputs this crop produces when it matures                         | no       |               |
| renderBlock | [Block](/vanilla/api/block/Block)                  | The block that should be rendered in world                             | no       |               |
| renderType  | string                                             | The render type that should be used                                    | sì       | "generic"     |
