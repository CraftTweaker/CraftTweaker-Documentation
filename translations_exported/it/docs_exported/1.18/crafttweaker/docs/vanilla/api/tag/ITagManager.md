...
| tagType | string                                                                              | sì        | no        | Get the tag type. In a Bracket call, this will used to determine which ITagManager to use. <br />   <br />  `<tag:{tag_type`:tag_location:tag_name>} <br> <br />  `<tagManager:{tag_type`>} |
