...
| ----------------- | ------- | --------- | --------- | ----------------------------------------------------------------------------------------------- |
| commandString     | string  | sì        | no        | Gets the command string for this EntityClassification                                           |
| isAnimal          | boolean | sì        | no        | Checks if this EntityClassification is an animal.                                               |
| isPeaceful        | boolean | sì        | no        | Checks if this EntityClassification is peaceful.                                                |
| maxNumberOfEntity | int     | sì        | no        | Gets how many Entities with this EntityClassification can be in the same area at the same time. |
| nome              | string  | sì        | no        | Gets the name of this Entity Classification                                                     |
