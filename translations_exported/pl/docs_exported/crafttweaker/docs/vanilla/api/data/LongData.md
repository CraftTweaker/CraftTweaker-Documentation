...
LongData implementuje następujące interfejsy. Oznacza to, że każda dostępna dla nich metoda może być również stosowana w tej klasie.
- [crafttweaker.api.data.IData](/vanilla/api/data/IData)
- [crafttweaker.api.data.INumberData](/vanilla/api/data/INumberData)

## Konstruktorzy
```zenscript
nowy crafttweaker.api.data.LongData(wewnętrzna długość);
```
| Parametr   | Typ    | Opis             |
| ---------- | ------ | ---------------- |
| wewnętrzna | długie | Nie podano opisu |
...
```zenscript
800000000.contains(dane jako crafttweaker.api.data.IData);
