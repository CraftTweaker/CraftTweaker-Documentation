...
MCIngredientConditioned implementuje następujące interfejsy. Oznacza to, że każda dostępna dla nich metoda może być również stosowana w tej klasie.
...
```zenscript
myMCIngredientConditioned.matches(stack as crafttweaker.api.item.IItemStack, ignoruj obrażenia jako boolean);
```
...
| warunek         | [crafttweaker.api.item.IIngredientTransformer](/vanilla/api/items/IIngredientTransformer)&lt;T&gt; | prawda         | fałszywy      |
