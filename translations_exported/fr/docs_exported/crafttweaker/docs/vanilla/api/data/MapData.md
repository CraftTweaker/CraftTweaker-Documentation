...
Checks if the Map contains the given key.
 ...
 Renvoie : `Vrai si la carte contient la clé`
...
```zenscript
myMapData.contains(données comme crafttweaker.api.data.IData);
myMapData.contains("Affichage");
```
...
Retrieves the value associated with the key
 ...
 Renvoie : `La valeur si présente, nulle`
...
Merges this map and the other map. If entries from this map and the other map share the values are tried to be merged. If that does not work, then the value from the other map is used.
 ...
 Renvoie : `Cette carte, après la fusion`

Type de retour : [crafttweaker.api.data.MapData](/vanilla/api/data/MapData)
...
Adds sets the value for the given key or creates a new entry if it did not exist before.
 ...
 Renvoie : `La valeur précédente si présente, nulle`
...
| keySet  | Définir&lt;String&gt; | true       | false      |
| size    | int                               | true       | false      |
