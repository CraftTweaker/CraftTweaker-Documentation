...
DoubleData implémente les interfaces suivantes. Cela signifie que toutes les méthodes disponibles peuvent également être utilisées dans cette classe.
...
| --------- | ------------- | -------------------------- |
| interne   | double        | Aucune description fournie |
...
```zenscript
3.25.contains(données comme crafttweaker.api.data.IData);
3.25.contains("Affichage");
```
