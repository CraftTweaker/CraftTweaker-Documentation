...
## Constructors
Creates a new BlockBuilder. これは _ではなく_ ゲーム内で新しいブロックを作成することに注意してください。そのためには、 [mods.contentweaker.block.BlockBuilder#build(String)](/mods/contenttweaker/API/block/BlockBuilder/#build) を呼び出す必要があります。
```zenscript
...
Instructs CoT that this block will does not block movement. [mods.contenttweaker.block.BlockBuilder#notSolid()](/mods/contenttweaker/API/block/BlockBuilder/#notsolid)
