...
2つの位置を追加し、結果を返します。
 ...
 戻り値: `new [crafttweaker.api.util.BlockPos](/vanilla/api/util/BlockPos) が追加された値を持つ。`

戻り値の型: [crafttweaker.api.util.BlockPos](/vanilla/api/util/BlockPos)
...
```zenscript
new BlockPos(0, 1, 2).add(int, y as int, z as int);
...
このポジションのクロス積と与えられた位置に基づいて、新しいBlockPosを作成します。
 ...
 戻り値: `この BlockPos と与えられた BlockPos のクロス積に基づく新しいBlockPos`
...
戻り値の型: net.minecraft.util.math.BlockPos
...
2 つの位置を一緒に減算し、結果を返します。
 ...
 戻り値: `new [crafttweaker.api.util.BlockPos](/vanilla/api/util/BlockPos) と削除された値。`
...
指定されたBlockPosがこの BlockPos の指定された距離内にあるかどうかをチェックします(これはBlockPosの中央を使用します)
 ...
 戻り値: `与えられたBlockPosがこの BlockPos の与えられた距離にある場合は true`
