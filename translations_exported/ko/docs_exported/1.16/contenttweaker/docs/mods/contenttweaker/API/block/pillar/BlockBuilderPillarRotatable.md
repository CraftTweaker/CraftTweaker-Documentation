...
통나무처럼 회전할 수 있는 블록을 만들 수 있는 특수 블록 제작기입니다.

 즉 이 제작기는 위와 아래에 하나의 텍스처를 사용하고 옆면에는 하나의 텍스처를 사용하는 블록을 만들 수 있습니다. 보통 이런 블록의 면을 지칭할 때는 이름에 `_end`나 `sides`가 따라붙습니다. 대부분의 경우와 마찬가지로, 기본적으로 샘플 이미지가 생성되어 제공됩니다.
