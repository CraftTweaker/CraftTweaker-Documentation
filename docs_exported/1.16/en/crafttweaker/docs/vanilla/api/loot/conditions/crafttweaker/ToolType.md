...
Builder to create a 'ToolType' loot condition.
 ...
 This condition checks the tool that the [LootContext](/vanilla/api/loot/LootContext) reports as having been used to
 break a block or perform any other action is of the given [ToolType](/vanilla/api/tool/ToolType).

 The condition then passes if and only if a tool of the given type has been used to break the block.

 If additional properties of the tool should be checked, combine this condition with a
 [MatchTool](/vanilla/api/loot/conditions/vanilla/MatchTool) condition without
 specifying the type.
