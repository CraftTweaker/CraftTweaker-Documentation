...
```zenscript
// IIngredientAny.addGlobalAttributeModifier(attribute as Attribute, uuid as stdlib.UUID, name as string, value as double, operation as AttributeOperation, slotTypes as EquipmentSlot[])
...
```zenscript
IIngredientAny.condition(condition as IIngredientCondition) as IIngredient
```
