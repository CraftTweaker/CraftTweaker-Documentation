...
```zenscript
// MCItemStack.addGlobalAttributeModifier(attribute as Attribute, uuid as invalid, name as string, value as double, operation as AttributeOperation, slotTypes as EquipmentSlot[]) as void
...
```zenscript
MCItemStack.clearTooltip(leaveName as boolean) as void
```
...
```zenscript
// MCItemStack.removeGlobalAttributeModifier(uuid as invalid, slotTypes as EquipmentSlot[]) as void
...
```zenscript
// MCItemStack.setBurnTime(time as int) as void
...
```zenscript
MCItemStack.setBurnTime(time as int, manager as IRecipeManager) as void
```
...
```zenscript
MCItemStack.setFood(food as FoodProperties?) as void
```
