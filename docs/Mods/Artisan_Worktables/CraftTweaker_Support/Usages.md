...
|可选      |GameStage:|Require GameStage(s)（需要的GameStage） |.requireGameStages|String require（匹配方式）, String[] stages	|require（匹配方式） = "ALL"（全部） 或 "ANY"（任意）|
|可选      |GameStage:|Exclude GameStage(s)（排除的GameStage） |.excludeGameStages				|String[] stages												 			|																 |
