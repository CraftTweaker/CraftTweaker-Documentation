...
| sendStatusMessage(message, hotbar) | the message is a string or an [IFormattedText](/Vanilla/Utils/IFormattedText/) | Sends the player a status message, if the hotbar argument is true, the message will be displayed on player's hotbar, the default value is true | `player.sendStatusMessage("hello, world")` |
