...
SlurryStack implements the following interfaces. That means all methods defined in these interfaces
are also available in SlurryStack
...
|-------------|-------------|
| CHEMICAL | true |
| [SlurryStackIngredient](/mods/Mekanism/api/ingredient/ChemicalStackIngredient/SlurryStackIngredient) | true |
...
```zenscript
// SlurryStack.asImmutable() as CRT_STACK
...
```zenscript
// SlurryStack.asMutable() as CRT_STACK
...
```zenscript
SlurryStack.containsOther(stack as CRT_STACK) as boolean
```
...
```zenscript
// SlurryStack.getRegistryName() as MCResourceLocation
...
```zenscript
// SlurryStack.getType() as CHEMICAL
...
```zenscript
SlurryStack.grow(amount as long) as CRT_STACK
```
...
```zenscript
// SlurryStack.isEmpty() as boolean
...
```zenscript
SlurryStack.isEqual(other as CRT_STACK) as boolean
```
...
```zenscript
SlurryStack.isTypeEqual(stack as CRT_STACK) as boolean
```
...
```zenscript
SlurryStack.setAmount(amount as long) as CRT_STACK
```
...
```zenscript
SlurryStack.shrink(amount as long) as CRT_STACK
```
...
```zenscript
stack as CRT_STACK in mySlurryStack
```
...
```zenscript
mySlurryStack == other as CRT_STACK
```
