...
```zenscript
import mods.mekanism.recipe.ItemStackChemicalToItemStack.Purifying;
```

## Extending ItemStackChemicalToItemStack

Purifying extends [ItemStackChemicalToItemStack](/mods/Mekanism/recipe/ItemStackChemicalToItemStack)
...
```zenscript
Purifying.addJSONRecipe(name as string, data as IData) as void
```
...
```zenscript
Purifying.addRecipe(name as string, itemInput as ItemStackIngredient, chemicalInput as INGREDIENT, output as IItemStack) as void
```
...
```zenscript
// Purifying.getAllRecipes() as stdlib.List<WrapperRecipe>

myPurifying.getAllRecipes();
```
...
```zenscript
Purifying.getRecipeByName(name as string) as WrapperRecipe
```
...
```zenscript
// Purifying.getRecipeMap() as WrapperRecipe[MCResourceLocation]

myPurifying.getRecipeMap();
```
...
```zenscript
// Purifying.removeAll() as void

myPurifying.removeAll();
```
...
```zenscript
Purifying.removeByModid(modid as string) as void
```
...
```zenscript
Purifying.removeByModid(modid as string, exclude as RecipeFilter) as void
```
...
```zenscript
Purifying.removeByName(name as string) as void
```
...
```zenscript
Purifying.removeByRegex(regex as string) as void
```
...
```zenscript
Purifying.removeByRegex(regex as string, exclude as RecipeFilter) as void
```
...
```zenscript
Purifying.removeRecipe(output as IItemStack) as void
```
...
```zenscript
Purifying.removeRecipeByInput(input as IItemStack) as void
```
