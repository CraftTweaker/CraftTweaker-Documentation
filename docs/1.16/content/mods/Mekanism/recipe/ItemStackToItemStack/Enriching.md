...
```zenscript
import mods.mekanism.recipe.ItemStackToItemStack.Enriching;
```

## Extending ItemStackToItemStack

Enriching extends [ItemStackToItemStack](/mods/Mekanism/recipe/ItemStackToItemStack). That means all
methods available in [ItemStackToItemStack](/mods/Mekanism/recipe/ItemStackToItemStack) are also
available in Enriching
...
```zenscript
Enriching.addJSONRecipe(name as string, data as IData) as void
```
...
```zenscript
Enriching.addRecipe(name as string, input as ItemStackIngredient, output as IItemStack) as void
```
...
```zenscript
// Enriching.getAllRecipes() as stdlib.List<WrapperRecipe>

myEnriching.getAllRecipes();
```
...
```zenscript
Enriching.getRecipeByName(name as string) as WrapperRecipe
```
...
```zenscript
// Enriching.getRecipeMap() as WrapperRecipe[MCResourceLocation]

myEnriching.getRecipeMap();
```
...
```zenscript
// Enriching.removeAll() as void

myEnriching.removeAll();
```
...
```zenscript
Enriching.removeByModid(modid as string) as void
```
...
```zenscript
Enriching.removeByModid(modid as string, exclude as RecipeFilter) as void
```
...
```zenscript
Enriching.removeByName(name as string) as void
```
...
```zenscript
Enriching.removeByRegex(regex as string) as void
```
...
```zenscript
Enriching.removeByRegex(regex as string, exclude as RecipeFilter) as void
```
...
```zenscript
Enriching.removeRecipe(output as IItemStack) as void
```
...
```zenscript
Enriching.removeRecipeByInput(input as IItemStack) as void
```
